...
function MenuContent({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="min-h-screen bg-white pb-24">
      {/* Banner - tam görünür */}
      {data.bannerUrl ? (
        <div className="relative w-full md:h-80 lg:h-96 overflow-hidden bg-white animate-fade-in">
        ...
        <div className="w-full h-40 md:h-56 bg-gradient-to-br from-gray-100 to-gray-50 flex items-center justify-center animate-fade-in">
          <svg className="w-12 h-12 text-gray-300 animate-breathe" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M4 6h16M4 12h16M4 18h16" /></svg>
        </div>
      )}
      ...
      {(masa || data.googleReviewUrl || data.instagramUrl) && (
        <div className="max-w-2xl mx-auto px-3 md:px-4 pt-4 flex items-center justify-between gap-3 flex-wrap animate-fade-in-up" style={{ animationDuration: '0.6s' }}>
          <div className="flex items-center gap-2">
            {masa ? (
              <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-black text-white text-xs md:text-sm font-bold shadow-md">
              ...
              <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-100 text-gray-700 text-xs md:text-sm font-bold">
          ...
          <div className="flex items-center gap-2">
            {data.googleReviewUrl && (
              <a href={data.googleReviewUrl} target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-full bg-white border border-gray-200 text-gray-700 text-xs font-semibold hover:shadow-md hover:-translate-y-0.5 transition-all active:scale-95">
      ...
      {/* Kategoriler */}
      {categories.length > 0 && (
        <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-xl border-b border-gray-200/70 px-3 md:px-4 py-2.5 md:py-3 overflow-x-auto mt-2 animate-fade-in" style={{ animationDuration: '0.5s' }}>
          <div className="flex gap-1.5 md:gap-2 max-w-2xl mx-auto">
            <button onClick={() => setSelectedCategory('')}
              className={'px-3 md:px-5 py-1.5 md:py-2 rounded-full text-[11px] md:text-xs font-semibold transition-all whitespace-nowrap ' + (!selectedCategory ? 'bg-black text-white shadow-md' : 'text-gray-700 hover:text-gray-900 bg-gray-100 hover:bg-gray-200')}>Tümü</button>
            {categories.map(c => (
              <button key={c} onClick={() => setSelectedCategory(c)}
                className={'px-3 md:px-5 py-1.5 md:py-2 rounded-full text-[11px] md:text-xs font-semibold transition-all whitespace-nowrap ' + (selectedCategory === c ? 'bg-black text-white shadow-md' : 'text-gray-700 hover:text-gray-900 bg-gray-100 hover:bg-gray-200')}>{c}</button>
            ))}
          </div>
        </div>
      )}
            ...
            return (
              <div key={p.id} style={{ animationDelay: `${Math.min(idx, 8) * 60}ms` }} onClick={() => { if (!isSoldout && !isPreparing) { setSelectedProduct(p); setQty(1) } }} className={'animate-fade-in-up flex flex-col rounded-xl md:rounded-2xl bg-white border shadow-sm transition-all overflow-hidden ' + (isSoldout || isPreparing ? 'opacity-60 border-gray-200 cursor-not-allowed' : 'border-gray-200 hover:shadow-lg hover:-translate-y-1 cursor-pointer active:scale-[0.98]')}>
                <div className="relative">
                  ...
                  {isSoldout && (
                    <span className="absolute top-2 left-2 px-2.5 py-1 rounded-full bg-black/70 backdrop-blur text-white text-[9px] md:text-[11px] font-bold uppercase tracking-wide">Tükendi</span>
                  )}
                  {isPreparing && (
                ...
                <div className="p-2.5 md:p-4 flex-1 flex flex-col">
                    ...
                    <div className="text-right flex-shrink-0">
                      {isDiscounted ? (
                        <div className="flex flex-col items-end">
                          <span className="text-gray-900 font-bold text-[11px] md:text-sm whitespace-nowrap">₺{p.price}</span>
                  ...
                  <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                    {p.weight && (
                      <span className="inline-flex items-center gap-1 px-1.5 md:px-2 py-0.5 rounded-full bg-gray-50 text-gray-600 text-[8px] md:text-[10px] font-semibold border border-gray-200">
                  ...
                  {isSoldout ? (
                    ...
                    <button
                      ...
                      className="mt-2.5 w-full py-2 rounded-xl bg-green-600 text-white text-[11px] md:text-xs font-semibold flex items-center justify-center gap-1.5 shadow-sm animate-pop-in">
                    ...
                    <button
                      ...
                      className="mt-2.5 w-full py-2 rounded-xl bg-black hover:bg-gray-800 active:scale-[0.98] transition-all text-white text-[11px] md:text-xs font-semibold flex items-center justify-center gap-1.5 shadow-sm shine">
      ...
      {/* Ürün Detay Modal */}
      {selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => { if (justAdded !== selectedProduct.id) setSelectedProduct(null) }}>
          <div className="relative w-full md:max-w-lg bg-white rounded-t-3xl md:rounded-3xl shadow-2xl overflow-hidden transition-all duration-200 animate-slide-up" onClick={e => e.stopPropagation()}>
            <div className="md:hidden h-1.5 w-12 bg-gray-200 rounded-full mx-auto mt-3" />
            <button onClick={() => setSelectedProduct(null)} className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full bg-white/80 backdrop-blur flex items-center justify-center text-gray-700 hover:bg-white shadow-sm transition-all">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
            {selectedProduct.image && (
              <div className="w-full aspect-square md:aspect-video bg-gray-50">
            ...
            <div className="p-5 md:p-6">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="text-right flex-shrink-0">
                  {(selectedProduct.originalPrice && parseFloat(selectedProduct.originalPrice) > parseFloat(selectedProduct.price)) ? (
                    <div className="flex flex-col items-end">
                      <span className="text-gray-900 font-bold text-lg md:text-xl whitespace-nowrap">₺{selectedProduct.price}</span>
              ...
              {selectedProduct.description && (
                <p className="text-sm md:text-base text-gray-500 mt-3 leading-relaxed pl-3 border-l-2 border-gray-200 font-medium">{selectedProduct.description}</p>
              )}

              {/* Miktar + Sepete Ekle */}
              <div className="flex items-center gap-3 mt-5">
                <div className="flex items-center gap-1 bg-gray-50 border border-gray-200 rounded-xl p-1">
                  <button onClick={() => setQty(q => Math.max(1, q - 1))} className="w-8 h-8 rounded-lg bg-white shadow-sm text-gray-800 font-bold text-lg hover:bg-gray-100 transition-colors active:scale-90">−</button>
                  <span className="w-8 text-center text-gray-900 font-bold text-sm">{qty}</span>
                  <button onClick={() => setQty(q => q + 1)} className="w-8 h-8 rounded-lg bg-white shadow-sm text-gray-800 font-bold text-lg hover:bg-gray-100 transition-colors active:scale-90">+</button>
                </div>
                {justAdded === selectedProduct.id ? (
                  <button
                    className="flex-1 py-3 rounded-xl bg-green-600 text-white text-sm font-semibold flex items-center justify-center gap-2 shadow-md animate-pop-in">
                  ...
                  <button
                    ...
                    className="flex-1 py-3 rounded-xl bg-black hover:bg-gray-800 active:scale-[0.98] transition-all text-white text-sm font-semibold flex items-center justify-center gap-2 shadow-md shine">
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-40 px-3 md:px-4 pb-3 md:pb-4 pointer-events-none animate-fade-in-up" style={{ animationDuration: '0.4s' }}>
          <button
            ...
            className="pointer-events-auto w-full max-w-2xl mx-auto flex items-center justify-between gap-3 px-4 md:px-5 py-3.5 rounded-2xl bg-black text-white shadow-2xl hover:bg-gray-900 active:scale-[0.99] transition-all">
      ...
      {/* Sepet Modal */}
      {showCart && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => setShowCart(false)}>
          <div className="relative w-full md:max-w-lg bg-white rounded-t-3xl md:rounded-3xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col transition-all duration-200 animate-slide-up" onClick={e => e.stopPropagation()}>
            <div className="md:hidden h-1.5 w-12 bg-gray-200 rounded-full mx-auto mt-3" />
            <div className="flex items-center justify-between px-5 md:px-6 pt-4 pb-3 border-b border-gray-100">
              <h2 className="text-gray-900 font-bold text-lg">Sepetim</h2>
              <button onClick={() => setShowCart(false)} className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-gray-700 hover:bg-gray-100 transition-all active:scale-90">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 md:px-6 py-4 space-y-3">
              ...
              {cart.map((item) => (
                <div key={item.product.id} className="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-2xl p-3 animate-fade-in-up" style={{ animationDuration: '0.35s' }}>
                  {item.product.image ? (
                  ...
                  <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-lg p-0.5">
                    <button onClick={() => updateQty(item.product.id, -1)} className="w-7 h-7 rounded-md text-gray-800 font-bold hover:bg-gray-100 transition-colors active:scale-90">−</button>
                    <span className="w-7 text-center text-gray-900 font-bold text-sm">{item.qty}</span>
                    <button onClick={() => updateQty(item.product.id, 1)} className="w-7 h-7 rounded-md text-gray-800 font-bold hover:bg-gray-100 transition-colors active:scale-90">+</button>
                  </div>
                  <div className="text-right flex-shrink-0 w-16">
                    <p className="text-gray-900 font-bold text-sm">₺{((parseFloat(item.product.price) || 0) * item.qty).toFixed(2)}</p>
            ...
            {cart.length > 0 && (
              <div className="px-5 md:px-6 py-4 border-t border-gray-100 bg-white">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-gray-500 text-sm">Toplam</span>
                  <span className="text-gray-900 font-bold text-xl">₺{cartTotal.toFixed(2)}</span>
                </div>
                <button
                  onClick={openCheckout}
                  className="w-full py-3.5 rounded-xl bg-black hover:bg-gray-800 active:scale-[0.99] transition-all text-white font-semibold text-sm flex items-center justify-center gap-2 shadow-md shine">
                  Ödemeye Geç
          ...
          <div className="relative w-full md:max-w-lg bg-white rounded-t-3xl md:rounded-3xl shadow-2xl overflow-hidden max-h-[92vh] flex flex-col transition-all duration-200 animate-slide-up">
            {orderResult ? (
              <div className="p-8 text-center">
                ...
                <p className="text-gray-500 text-sm mt-2">Sipariş numaranız: <span className="font-bold text-gray-900">#{orderResult.id}</span></p>
                ...
                <button
                  ...
                  className="mt-6 w-full py-3 rounded-xl bg-black hover:bg-gray-800 active:scale-[0.99] transition-all text-white font-semibold text-sm shine">
                  Menüye Dön
                </button>
              </div>
            ) : (
              <>
                ...
                <div className="flex items-center justify-between px-5 md:px-6 pt-4 pb-3 border-b border-gray-100">
                  ...
                  <button onClick={() => setCheckoutOpen(false)} className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-gray-700 hover:bg-gray-100 transition-all active:scale-90">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-5 md:px-6 py-4 space-y-4">
                  {/* Sipariş Özeti */}
                  <div className="bg-gray-50 border border-gray-100 rounded-2xl p-4 animate-fade-in-up" style={{ animationDuration: '0.35s' }}>
                    <p className="text-gray-500 text-[11px] font-semibold uppercase tracking-wide mb-2">Sipariş Özeti</p>
                    ...
                    <div className="flex justify-between text-sm mt-3 pt-3 border-t border-gray-200">
                      ...
                      <span className="text-gray-900 font-bold text-base">₺{cartTotal.toFixed(2)}</span>
                    </div>
                  </div>

                  {/* İletişim Bilgileri */}
                  <div className="animate-fade-in-up" style={{ animationDuration: '0.4s' }}>
                    <p className="text-gray-500 text-[11px] font-semibold uppercase tracking-wide mb-2">İletişim Bilgileri</p>
                    <div className="space-y-3">
                      <input
                        value={form.name}
                        onChange={e => setForm({ ...form, name: e.target.value })}
                        placeholder="Ad Soyad *"
                        className="w-full px-4 py-3 rounded-xl bg-white border border-gray-200 focus:border-gray-400 focus:ring-2 focus:ring-gray-100 outline-none text-gray-900 text-sm placeholder:text-gray-400 transition-all"
                      />
                      <input
                        value={form.phone}
                      ...
                      {!masa && (
                        <textarea
                          ...
                          rows={3}
                          className="w-full px-4 py-3 rounded-xl bg-white border border-gray-200 focus:border-gray-400 focus:ring-2 focus:ring-gray-100 outline-none text-gray-900 text-sm placeholder:text-gray-400 transition-all resize-none"
                        />
                      )}
                      {masa && (
                        <p className="text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 flex items-center gap-2">
                  ...
                  <div className="animate-fade-in-up" style={{ animationDuration: '0.45s' }}>
                    <p className="text-gray-500 text-[11px] font-semibold uppercase tracking-wide mb-2">Ödeme Yöntemi</p>
                    <div className="grid grid-cols-1 gap-2">
                      {payments.map((p) => (
                        <label key={p} onClick={() => setForm({ ...form, payment: p })}
                          className={'flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition-all ' + (form.payment === p ? 'bg-gray-50 border-gray-900 ring-2 ring-gray-100' : 'bg-white border-gray-200 hover:border-gray-300')}>
                          <span className={'w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all ' + (form.payment === p ? 'border-gray-900' : 'border-gray-300')}>
                            {form.payment === p && <span className="w-2 h-2 rounded-full bg-gray-900 animate-pop-in" />}
                          </span>
                ...
                <div className="px-5 md:px-6 py-4 border-t border-gray-100 bg-white">
                  <button
                    ...
                    className="w-full py-3.5 rounded-xl bg-black hover:bg-gray-800 active:scale-[0.99] transition-all text-white font-semibold text-sm flex items-center justify-center gap-2 shadow-md disabled:opacity-40 disabled:cursor-not-allowed shine">
                    {submitting ? (
...
export default function MenuPage({ params }: { params: { slug: string } }) {
  return (
    <Suspense fallback={<div className="min-h-screen bg-white flex items-center justify-center"><div className="w-10 h-10 border-2 border-gray-900 border-t-transparent rounded-full animate-spin" /></div>}>
