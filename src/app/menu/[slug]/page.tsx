        ...
        <div className="max-w-2xl mx-auto px-3 md:px-4 pt-4 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            {masa ? (
              ...
              <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-100 text-amber-800 text-xs md:text-sm font-bold">
            ...
            return (
              <div key={p.id} onClick={() => { if (!isSoldout && !isPreparing) { setSelectedProduct(p); setQty(1) } }} className={'flex flex-col rounded-xl md:rounded-2xl bg-white border shadow-sm transition-all overflow-hidden ' + (isSoldout || isPreparing ? 'opacity-75 border-amber-200/40 cursor-not-allowed' : 'border-amber-100/60 hover:shadow-md cursor-pointer active:scale-[0.98] transition-transform')}>
                <div className="relative">
                  ...
                  {isPreparing && (
                    <span className="absolute top-2 left-2 px-2.5 py-1 rounded-full bg-amber-500/90 backdrop-blur text-white text-[9px] md:text-[11px] font-bold uppercase tracking-wide">Hazırlıkta</span>
                  )}
                </div>
                <div className="p-2.5 md:p-4 flex-1 flex flex-col">
                  <div className="flex items-start justify-between gap-1.5">
                    <h3 className={'font-semibold text-[11px] md:text-sm leading-tight flex-1 ' + (isSoldout ? 'text-amber-950/50 line-through' : 'text-amber-950')}>{p.name}</h3>
                  ...
                  {p.description && (
                    <p className={'text-[9px] md:text-[11px] mt-1.5 leading-relaxed line-clamp-2 ' + (isSoldout ? 'text-amber-800/40' : 'text-amber-800/70')}>{p.description}</p>
                  )}
                  {isSoldout ? (
                    ...
                    <div className="mt-2.5 w-full py-2 rounded-xl bg-amber-100/70 text-amber-600 text-[11px] md:text-xs font-semibold flex items-center justify-center gap-1.5 cursor-not-allowed">
          ...
          <div className="relative w-full md:max-w-lg bg-white rounded-t-3xl md:rounded-3xl shadow-2xl overflow-hidden max-h-[92vh] flex flex-col transition-all duration-200">
            {orderResult ? (
              <div className="p-8 text-center">
                ...
                {masa && <p className="text-amber-800/70 text-xs mt-1">Masa {masa} numaralı masanıza en kısa sürede servis edilecektir.</p>}
                {!masa && <p className="text-amber-800/70 text-xs mt-1">İşletme siparişinizi en kısa sürede hazırlayacaktır.</p>}
                <button
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto px-5 md:px-6 py-4 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      ...
                      {masa && (
                        <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200/60 rounded-xl px-4 py-3 flex items-center gap-2">
                ...
                <div className="px-5 md:px-6 py-4 border-t border-amber-100/60 bg-[#fdfbf7]">
                  ...
                  <p className="text-center text-[10px] text-amber-400 mt-2">{masa ? 'Ödeme masada teslim sırasında yapılacaktır.' : 'Ödeme teslimat sırasında yapılacaktır.'}</p>
                </div>
              </>
            )}
          </div>
...
export default function MenuPage({ params }: { params: { slug: string } }) {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#f8f6f1] flex items-center justify-center"><div className="w-10 h-10 border-2 border-amber-600 border-t-transparent rounded-full animate-spin" /></div>}>
