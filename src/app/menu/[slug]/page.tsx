...
function MenuContent({ params }: { params: { slug: string } }) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#f4f7fd] flex items-center justify-center">
  ...
  const brand = data.primaryColor || '#2563eb'
  const brand2 = data.secondaryColor || '#1d4ed8'
  const gBrand: any = { background: `linear-gradient(135deg, ${brand}, ${brand2})`, boxShadow: `0 12px 28px -8px ${brand}88` }
  const gSoft: any = { background: brand + '14' }
  const gBorder: any = { borderColor: brand + '40' }

  return (
    <div className="relative min-h-screen bg-[#f4f7fd] pb-28 overflow-x-hidden" style={{ ['--br' as any]: brand, ['--br2' as any]: brand2 }}>
      {/* Arka plan süslemeleri */}
      <div className="fixed inset-0 pointer-events-none z-0" aria-hidden="true">
        <div className="absolute -top-24 -right-24 w-[28rem] h-[28rem] rounded-full blur-3xl" style={{ background: brand + '22' }} />
        <div className="absolute top-1/3 -left-40 w-[26rem] h-[26rem] rounded-full blur-3xl" style={{ background: brand2 + '1f' }} />
        <div className="absolute -bottom-40 right-1/4 w-[30rem] h-[30rem] rounded-full blur-3xl" style={{ background: brand + '14' }} />
      </div>

      <div className="relative z-10">
        {/* Banner - tam görünür */}
        {data.bannerUrl ? (
          <div className="relative w-full md:h-72 lg:h-80 overflow-hidden bg-gradient-to-b from-blue-100 to-transparent animate-fade-in">
            ...
            <div className="absolute inset-x-0 bottom-0 h-24 md:h-32 bg-gradient-to-t from-[#f4f7fd] to-transparent" />
          </div>
        ) : (
          <div className="relative w-full h-48 md:h-60 overflow-hidden animate-fade-in" style={gBrand}>
            <div className="absolute -top-16 -right-16 w-64 h-64 rounded-full bg-white/10 blur-3xl" />
            <div className="absolute -bottom-20 -left-10 w-72 h-72 rounded-full bg-black/10 blur-3xl" />
            <div className="relative flex flex-col items-center justify-center h-full gap-2.5">
        ...
        {(data.shopName || data.address || data.phone || (data.workingHours && data.workingHours.length) || (data.paymentMethods && data.paymentMethods.length)) && (
          <div className="max-w-2xl mx-auto px-3 md:px-4 -mt-6 relative z-10">
            <div className="relative overflow-hidden bg-white border border-blue-100 rounded-2xl shadow-xl shadow-blue-600/5 p-4 md:p-5 animate-slide-up" style={{ animationDuration: '0.5s' }}>
              <div className="absolute -top-8 -right-6 w-28 h-28 rounded-full bg-blue-50/80 blur-2xl" />
              ...
              {data.workingHours && data.workingHours.length > 0 && (
                <div className="relative mt-3 pt-3 border-t border-blue-50">
        ...
        {(masa || data.googleReviewUrl || data.instagramUrl) && (
          <div className="max-w-2xl mx-auto px-3 md:px-4 pt-4 flex items-center justify-between gap-3 flex-wrap animate-fade-in-up" style={{ animationDuration: '0.6s' }}>
            <div className="flex items-center gap-2 flex-wrap">
              {masa ? (
                <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-white text-xs md:text-sm font-bold shadow-lg transition-all" style={gBrand}>
        ...
        {/* Kategoriler */}
        {categories.length > 0 && (
          <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-xl border-b border-blue-100 px-3 md:px-4 py-2.5 md:py-3 overflow-x-auto no-scrollbar mt-2 animate-fade-in shadow-sm" style={{ animationDuration: '0.5s' }}>
            <div className="flex gap-1.5 md:gap-2 max-w-2xl mx-auto">
              <button onClick={() => setSelectedCategory('')}
                style={!selectedCategory ? gBrand : undefined}
                className={'px-3 md:px-5 py-1.5 md:py-2 rounded-full text-[11px] md:text-xs font-semibold transition-all whitespace-nowrap ' + (!selectedCategory ? 'text-white' : 'bg-white text-blue-700 border border-blue-200 hover:bg-blue-50 hover:border-blue-300')}>Tümü</button>
              {categories.map(c => (
                <button key={c} onClick={() => setSelectedCategory(c)}
                  style={selectedCategory === c ? gBrand : undefined}
                  className={'px-3 md:px-5 py-1.5 md:py-2 rounded-full text-[11px] md:text-xs font-semibold transition-all whitespace-nowrap ' + (selectedCategory === c ? 'text-white' : 'bg-white text-blue-700 border border-blue-200 hover:bg-blue-50 hover:border-blue-300')}>{c}</button>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="max-w-2xl mx-auto px-3 md:px-4 py-4 md:py-6">
          {filtered.length === 0 && (
            <div className="text-center py-12 md:py-16 animate-fade-in">
              <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-white border border-dashed border-blue-200 flex items-center justify-center">
                <svg className="w-7 h-7 text-blue-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 6h16M4 12h16M4 18h16" /></svg>
              </div>
                  ...
                  <div className="p-2.5 md:p-4 flex-1 flex flex-col">
                      ...
                      <div className="text-right flex-shrink-0">
                        {isDiscounted ? (
                          <div className="flex flex-col items-end">
                            <span className="text-blue-700 font-bold text-[11px] md:text-sm whitespace-nowrap tabular-nums">₺{p.price}</span>
                    ...
                    {isSoldout ? (
                      ...
                      <button
                        ...
                        style={gBrand}
                        className="mt-2.5 w-full py-2 rounded-xl text-white text-[11px] md:text-xs font-semibold flex items-center justify-center gap-1.5 hover:scale-[1.03] active:scale-[0.97] transition-all shine">
              ...
              <div className="p-5 md:p-6">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right flex-shrink-0">
                    {(selectedProduct.originalPrice && parseFloat(selectedProduct.originalPrice) > parseFloat(selectedProduct.price)) ? (
                      <div className="flex flex-col items-end">
                        <span className="text-blue-700 font-bold text-lg md:text-xl whitespace-nowrap tabular-nums">₺{selectedProduct.price}</span>
                ...
                <div className="flex items-center gap-3 mt-5">
                  <div className="flex items-center gap-1 bg-blue-50 border border-blue-100 rounded-xl p-1">
                    ...
                    <span className="w-8 text-center text-gray-900 font-bold text-sm tabular-nums">{qty}</span>
                  ...
                  {justAdded === selectedProduct.id ? (
                    ...
                    <button
                      ...
                      style={gBrand}
                      className="flex-1 py-3 rounded-xl text-white text-sm font-semibold flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all shine">
        ...
        {masa && (
          <div className="fixed bottom-24 md:bottom-28 right-3 md:right-6 z-40 animate-fade-in-up" style={{ animationDuration: '0.6s' }}>
            <button onClick={() => { setWaiterOpen(true); setWaiterSent(false) }}
              style={gBrand}
              className="flex items-center gap-2 pl-4 pr-5 py-3 rounded-full text-white text-sm font-semibold hover:scale-105 active:scale-95 transition-all shine">
                ...
                <>
                  ...
                  <div className="px-5 md:px-6 pb-6">
                    ...
                    <button onClick={callWaiter} disabled={waiterSending}
                      style={gBrand}
                      className="mt-4 w-full py-3.5 rounded-xl text-white font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed hover:scale-[1.01] active:scale-[0.99] transition-all shine">
                      {waiterSending ? (
        ...
        {cart.length > 0 && (
          <div className="fixed bottom-0 left-0 right-0 z-40 px-3 md:px-4 pb-3 md:pb-4 pointer-events-none animate-fade-in-up" style={{ animationDuration: '0.4s' }}>
            <button
              ...
              className="pointer-events-auto w-full max-w-2xl mx-auto flex items-center justify-between gap-3 px-4 md:px-5 py-3.5 rounded-2xl bg-slate-900 text-white shadow-2xl shadow-blue-900/30 hover:scale-[1.01] active:scale-[0.99] transition-all">
              ...
              <span className="font-bold text-sm md:text-base tabular-nums">₺{cartTotal.toFixed(2)}</span>
            </button>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4 bg-blue-950/50 backdrop-blur-sm animate-fade-in" onClick={() => setShowCart(false)}>
            <div className="relative w-full md:max-w-lg bg-white rounded-t-3xl md:rounded-3xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col transition-all duration-200 animate-slide-up" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex-1 overflow-y-auto px-5 md:px-6 py-4 space-y-3">
                ...
                {cart.map((item) => (
                  <div key={item.product.id} className="bg-gradient-to-r from-blue-50/80 to-white border border-blue-100 rounded-2xl p-3 animate-fade-in-up hover:shadow-md hover:shadow-blue-600/5 transition-all" style={{ animationDuration: '0.35s' }}>
                    <div className="flex items-center gap-3">
                    {item.product.image ? (
                    ...
                    <div className="flex items-center gap-1 bg-white border border-blue-200 rounded-lg p-0.5">
                      ...
                      <span className="w-7 text-center text-gray-900 font-bold text-sm tabular-nums">{item.qty}</span>
                      <button onClick={() => updateQty(item.product.id, 1)} className="w-7 h-7 rounded-md text-blue-700 font-bold hover:bg-blue-50 transition-colors active:scale-90">+</button>
                    </div>
                    <div className="text-right flex-shrink-0 w-16">
                      <p className="text-blue-700 font-bold text-sm tabular-nums">₺{((parseFloat(item.product.price) || 0) * item.qty).toFixed(2)}</p>
              ...
              {cart.length > 0 && (
                <div className="px-5 md:px-6 py-4 border-t border-blue-100 bg-white">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-blue-700 font-bold text-xl tabular-nums">₺{cartTotal.toFixed(2)}</span>
                  </div>
                  <button
                    onClick={openCheckout}
                    style={gBrand}
                    className="w-full py-3.5 rounded-xl text-white font-semibold text-sm flex items-center justify-center gap-2 hover:scale-[1.01] active:scale-[0.99] transition-all shine">
                    Ödemeye Geç
              ...
              {orderResult ? (
                <div className="p-8 text-center overflow-y-auto max-h-[85vh]">
                  ...
                  {orderResult.trackingCode && (
                    <div className="mt-5 rounded-2xl bg-gradient-to-r from-blue-50/80 to-white border border-blue-100 p-5 animate-fade-in-up" style={{ animationDuration: '0.4s' }}>
                      ...
                      <div className="flex items-center justify-center gap-3 mt-3">
                        <span className="text-3xl font-black tracking-[0.3em] text-blue-700 tabular-nums">{orderResult.trackingCode}</span>
                      </div>
                      <button
                  ...
                  <button
                    ...
                    style={gBrand}
                    className="mt-6 w-full py-3 rounded-xl text-white font-semibold text-sm hover:scale-[1.01] active:scale-[0.99] transition-all shine">
                    Menüye Dön
                  </button>
                </div>
              ) : (
                <>
                  ...
                  <div className="flex-1 overflow-y-auto px-5 md:px-6 py-4 space-y-4">
                    ...
                    <div className="bg-gradient-to-r from-blue-50/80 to-white border border-blue-100 rounded-2xl p-4 animate-fade-in-up" style={{ animationDuration: '0.35s' }}>
                        ...
                        {cart.map((item) => (
                          <div key={item.product.id} className="text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="text-gray-900 font-semibold tabular-nums">₺{((parseFloat(item.product.price) || 0) * item.qty).toFixed(2)}</span>
                            </div>
                      ...
                      <div className="flex justify-between text-sm mt-3 pt-3 border-t border-blue-100">
                        ...
                        <span className="text-blue-700 font-bold text-base tabular-nums">₺{cartTotal.toFixed(2)}</span>
                      </div>
                    </div>
                  ...
                  <div className="px-5 md:px-6 py-4 border-t border-blue-100 bg-white">
                    <button
                      ...
                      style={gBrand}
                      className="w-full py-3.5 rounded-xl text-white font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed hover:scale-[1.01] active:scale-[0.99] transition-all shine">
                      {submitting ? (
                        <span className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                      ) : (
...
export default function MenuPage({ params }: { params: { slug: string } }) {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#f4f7fd] flex items-center justify-center"><div className="w-10 h-10 border-2 border-blue-600 border-t-transparent rounded-full animate-spin" /></div>}>
